
# KEMBOI K. DAVID
## Data Analyst | Business Analyst | Cloud Data Analyst

📍 Westland, Nairobi  
📞 +254 724 006 182  
📧 Dkemboi888@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com/in/davidkemboi)  
🌐 [Portfolio](https://davido200.github.io/David_Portfolio/)

---

## Professional Profile

Data Analyst with **3+ years of experience** delivering end-to-end analytics solutions across **healthcare, consulting, and public-sector environments**. Strong background in **Business Intelligence, Cloud Data Platforms, ETL pipelines, and dashboarding**. Proven ability to transform raw data into actionable insights using **SQL, Python, Power BI, Tableau, and AWS**. Adept at data quality management, stakeholder collaboration, and building scalable analytics solutions that drive business and operational impact.

---

## Core Competencies

- Data Analysis & Statistical Modeling  
- Business Intelligence & Dashboarding  
- Cloud Data Analytics (AWS)  
- ETL & Data Pipelines  
- Data Quality, Validation & Governance  
- Stakeholder Reporting & Decision Support  

---

## Technical Skills

**Data Analysis & Processing:** Python (Pandas, NumPy), SQL, Excel  
**Visualization & BI:** Power BI, Tableau, Excel  
**Databases:** MySQL, PostgreSQL, SQL Server, Google BigQuery, AWS Aurora  
**Cloud & ETL:** AWS (Glue, Data Pipeline), Airbyte  
**Programming & Web:** Python, JavaScript, PHP, HTML  
**Version Control:** Git  

---

## Professional Experience

### AMREF Kenya — *Data Analyst*  
**Turkana County | Nov 2024 – May 2025**

- Delivered analytics insights that improved data-driven decision-making by **20%** across health programs  
- Implemented advanced data quality and validation checks, achieving **98% data accuracy**  
- Designed interactive **Power BI dashboards** to support program monitoring and executive reporting  
- Built and optimized **ETL pipelines**, reducing data processing time by **50%**  
- Collaborated with healthcare professionals and policymakers to increase evidence-based strategy adoption  
- Trained staff on analytics tools, improving organizational data literacy by **35%**  

---

### Consultant — *Data Analyst*  
**Nakuru County | Sep 2021 – Nov 2024**

- Automated data workflows, reducing manual reporting effort by **40%**  
- Cleaned, transformed, and standardized datasets, improving data consistency by **65%**  
- Developed BI dashboards in **Tableau and Power BI** for operational and management insights  
- Ensured analytics-ready datasets through strong data governance and integrity practices  
- Performed advanced analysis using **SQL, Python (Pandas, NumPy)**  
- Produced executive-ready analytical reports supporting strategic business decisions  

---

### Murang’a County Health — *Data Analyst (Attachment)*  
**Murang’a County | May 2019 – Apr 2020**

- Maintained **90%+ data accuracy** through rigorous extraction and cleaning processes  
- Built BI documentation, training materials, and reporting templates  
- Analyzed large healthcare datasets to support hospital management decision-making  
- Ensured consistency and reliability of hospital performance metrics  

---

## Education

**Certificate in Data Analysis** — *Distinction*  
Turing College, Lithuania | 2024

**Bachelor of Business Information Technology** — *Second Class Upper Division*  
Mount Kenya University, Thika | 2020 – 2024

**Diploma in Business Information Technology** — *Credit*  
Mount Kenya University, Thika | 2018 – 2019

---

## Certifications

- AWS Certified Cloud Practitioner  
- SQL for Data Analysis — LinkedIn Learning, 365 Data Science  
- Data Analytics — LinkedIn Learning  
- Python Programmer Bootcamp — 365 Data Science  
- Data Visualization Certification  
- SQL + Tableau + Python Certification  

---

## Selected Projects

- **Machine Learning:** Linear Regression Model  
- **Healthcare Analytics:** Health Centre Data Analysis (Python, Pandas, NumPy)  
- **Business Intelligence:** Call Centre & HR Dashboards (Power BI)  
- **Data Cleaning:** FIFA 21 Dataset  
- **Customer Analytics:** Restaurant Customer Behavior Analysis (SQL)

🔗 **GitHub & Project Links:**  
- https://github.com/davido200  
- https://davido200.github.io/David_Portfolio/#projects

